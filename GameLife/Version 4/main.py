...
pausa = False #control de la ejecucion en el juego
...
while count < myKeyboard: #Bucles que recorren cada una de las celdas
    ...
    # Ciclo For para pausa por teclado
    for event in pulsaciones:
        if event.type ==pygame.KEYDOWN:
            pausa = not pausa
        clickMouse= pygame.mouse.get_pressed() #Detecta si se oprime el mouse
        
        if sum(clickMouse)> 0 :
            posX, posY = pygame.mouse.get_pos()
            celX, celY = int(np.floor(posX / ancho )), int(np.floor(posY/altura))
            nuevasgameState[celX, celY] = not clickMouse[2]               
